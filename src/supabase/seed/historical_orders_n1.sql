...
sku_pool AS (
  SELECT
    ...
    CASE
      WHEN lower(COALESCE(p.name, '')) LIKE '%lampe%'
        OR lower(COALESCE(p.name, '')) LIKE '%applique%'
        OR lower(COALESCE(p.name, '')) LIKE '%suspension%' THEN 'lamps'
      WHEN lower(COALESCE(p.name, '')) LIKE '%tapis%' THEN 'rugs'
      WHEN lower(COALESCE(p.name, '')) LIKE '%miroir%' THEN 'mirrors'
      WHEN lower(COALESCE(p.name, '')) LIKE '%chaise%'
        OR lower(COALESCE(p.name, '')) LIKE '%fauteuil%' THEN 'chairs'
      WHEN lower(COALESCE(p.name, '')) LIKE '%bureau%' THEN 'desks'
      WHEN lower(COALESCE(p.name, '')) LIKE '%etagere%'
        OR lower(COALESCE(p.name, '')) LIKE '%bibliotheque%'
        OR lower(COALESCE(p.name, '')) LIKE '%bibliothèque%' THEN 'shelves'
